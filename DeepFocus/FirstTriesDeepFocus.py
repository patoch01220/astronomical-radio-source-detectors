...
train_RealDetection = myUtils.loadAllCatFile("./Dataset.nosync/Sources_detection/cat", "gaussians_", indices_train, keys)
validation_RealDetection = myUtils.loadAllCatFile("./Dataset.nosync/Sources_detection/cat", "gaussians_", indices_validation, keys)
test_RealDetection = myUtils.loadAllCatFile("./Dataset.nosync/Sources_detection/cat", "gaussians_", indices_test, keys)
...
def trainingModel(input, target, model, optimizer, device, criterion, epochs, batch_size, hidden_channels, lr, weight_decay=0, modelName='modeltmp', Training = True):
    ...
    if Training:
        ...
        torch.save({
            ...
            }, './trainedModels/'+modelName+'_lr'+str(lr).replace('.', '')+'.pt'
        )
        return model, optimizer, epochs, batch_size, hidden_channels, lossHistory
    else:
        print("Testing...")

        loadedModel = torch.load('./trainedModels/'+modelName+'_lr'+str(lr).replace('.', '')+'.pt')
...
train_binaryMaps = train_binaryMaps.to(device)
...
for lr in [0.0001, 0.001, 0.01, 0.1]:
    ...
    model, optimizer, epochs, batch_size, hidden_channels, lossHistory = trainingModel(train_input, train_binaryMaps, model, optimizer, device, criterion, epochs, batch_size, hidden_channels, lr, modelName='model_validation', Training=True)
    # print()
    # model.eval()
