...
class Encoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # x = self.b4(skip3)
        # print("x1: ", x.shape)
        x = torch.flatten(skip2, start_dim=1)
        # print("x2: ", x.shape)
        x = self.dense(x)
...
class Decoder(nn.Module):
    def __init__(self, hidden_c):
        ...
        self.dense = nn.Sequential(
            nn.Linear(hidden_c, 16 * 128 * 128),
            nn.LeakyReLU()
        )
        self.unflatten = nn.Unflatten(dim=1, unflattened_size=(16, 128, 128))
...
class BlobsFinder3(nn.Module):
    def __init__(self, hidden_c):
        super(BlobsFinder3, self).__init__()
