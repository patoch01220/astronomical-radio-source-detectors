...
parser.add_argument('--hidden_channels', default=128,  type=int, help='')

parser.add_argument('--is_stochastic', default=True,  type=int, help='...')
parser.add_argument('--noise_std', default=0.2,  type=float, help='...')

parser.add_argument("--save_path", default='./Results/plots2', type=str, help="....")
# ------------------------------------------------------ #
...
def trainingModel(dataloader, device, criterion, args, Training=True, modelName=""):
    ...
    model = BlobsFinder3(args.hidden_channels)
    # summary(model, (1, 512, 512))s
            ...
            if e > 150:
                if e % 20 == 0:
                    torch.save({
                        ...
                        }, './trainedModels/final_woutBN/'+ modelName+'_final_woutBN_e'+str(e)+'.pt'
                    )
        # --- Saving the model --- #     
        torch.save({
            'epoch': args.epochs,
            ...
            'optimizer_state_dict': optimizer.state_dict(),
            'train_loss_history': lossHistory,
            }, './trainedModels/final_woutBN/'+ modelName+'_final_woutBN_2'+'.pt'
        )
        return model, lossHistory
    else:
        print("Testing...")

        loadedModel = torch.load('./trainedModels/'+modelName+'_final2'+'.pt')
